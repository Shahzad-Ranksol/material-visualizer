...
- `npm run test:e2e` — Playwright (`tests/e2e/`): `golden.spec.ts` checks the WebGL renderer on synthetic scenes (outside-mask/occluder preservation, perspective, physical scale, colour ΔE2000, seams, determinism, safe failure); `studio-flow.spec.ts` runs upload → surface → review → material → compare → download with the real models (~2 min; downloads its room photo into the git-ignored `tests/e2e/.cache/`)
